					...
					int expected_status)
{
	...
	if (list == NULL && write(2, "load_test got NULL list\n", 24))
		exit(-1);
	if (name == NULL && write(2, "load_test got NULL name\n", 24))
		exit(-1);
	if (test_func == NULL && write(2, "load_test got NULL test_func\n", 29))
		exit(-1);
	if ((out = (t_unit_test*)malloc(sizeof(t_unit_test))) == NULL &&
			write(2, "load_test's call to malloc returned NULL\n", 41))
		exit(-1);
	out->name = name;
