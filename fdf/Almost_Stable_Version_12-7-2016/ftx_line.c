...
/*   Updated: 2016/07/08 19:28:15 by hmartzol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_point	*sf_iintersection(t_point a, t_point b, t_point c, t_point d)
{
	...
	e = ft_point(b.x - a.x, b.y - a.y);
	f = ft_point(d.x - c.x, d.y - c.y);
	h = f.x * -e.y + f.y * e.x;
	if (h <= 0)
		return (0);
	h = (float)((a.x - c.x) * -e.y + (a.y - c.y) * e.x) / h;
	if (h > 1 || h <= 0)
		return (0);
	if ((out = (t_point*)malloc(sizeof(t_point))) == NULL)
		return ((t_point *)-1);
	ft_ptinit(out, c.x + f.x * h, c.y + f.y * h);
	return (out);
}

/*
** flag based global data struct accesor
** flag set has GDX_NEW: (alloc a new struct)
**   ftx_data will return NULL on failure, or a pointer on succes.
** flag set has GDX_ACCES: (return the current struct)
**   ftx_data will return NULL on failure, or a pointer on succes.
** flag set has GDX_FREE: (free the struct)
**   ftx_data will return NULL on succes, or a non 0 on failure
*/
...
void	ftx_horizontal_line(t_image *img, t_point a, t_point b, t_point color)
{
	...
	{
		__asm__("bswap %0" : "=r" (color.x) : "0" (color.x));
		__asm__("bswap %0" : "=r" (color.y) : "0" (color.y));
	}
