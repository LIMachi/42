...
/*   Updated: 2017/01/26 01:52:42 by pbondoer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
							...
							int mask)
{
	...
	if (out == NULL || img == NULL || pos.x >= out->size.x || pos.x >=
			out->size.x || pos.x + img->size.x < 0 || pos.y + img->size.y < 0)
		return (out);
	if (mask != NOMASK && (LOCAL_ENDIAN != BIG_ENDIAN))
		mask = ft_bswap32(mask);
	l.x = (pos.x + img->size.x >= out->size.x) ?
		MAX(img->size.x, out->size.x) - pos.x : img->size.x;
	l.y = (pos.y + img->size.y >= out->size.y) ?
		MAX(img->size.y, out->size.y) - pos.y : img->size.y;
